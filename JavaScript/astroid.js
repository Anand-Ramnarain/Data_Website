...
const gPie = svgPie
  ...
  .attr(
    "transform",
    `translate(${+svgPie.attr("width") / 2}, ${+svgPie.attr("height") / 2})`
  );
...
function fetchDataPie(date) {
  ...
  fetch(url)
    .then((r) => r.json())
    .then((d) =>
      renderPieChart(categorizeAsteroids(d.near_earth_objects[date]))
    );
}
...
function drawPieChartTexts(arcs, arcGen) {
  ...
  gPie
    ...
    .text((d) => `${labels[d.index]}: ${d.data[1]}`);
}
...
function extractAsteroidData(data) {
  ...
  for (let date in data.near_earth_objects) {
    ...
    const distances = data.near_earth_objects[date].map((a) =>
...
function getDotColor(d, maxCount, minCount) {
  const dangerDistanceThreshold = 500000; 

  if (d.avgDistance < dangerDistanceThreshold) {
      return "orange"; 
